...
async function assertThrows(fn: () => Promise<unknown> | unknown, errorClass: any, msgIncludes?: string): Promise<void> {
  try {
    await fn();
    throw new Error(`Expected ${errorClass.name} to be thrown, but nothing was thrown`);
  } catch (error: any) {
    if (!(error instanceof errorClass)) {
      throw new Error(`Expected ${errorClass.name} to be thrown, but got ${error.constructor.name}`);
    }
